...
### Core building blocks (quick tour)

- Navigation: `resource.element.subElement`
- Filtering and selection: `where(predicate)`, `exists()`, `count()`
- Type handling: `is(Type)`, `as(Type)` for polymorphic fields
- Reference resolution: `resolve()` to dereference `Reference` elements
- String and set ops: `contains`, `startsWith`, `in`, `union`

FHIRPath is not a replacement for all business logic. An environment as complex as DEMIS needs databases, calculating
...
Bundle.entry.resource
  ...
  .where(code.coding.where(system = 'http://loinc.org' and code = '94500-6').exists())


Presence and counting
// Patient has an official name
Patient.name.where(use = 'official').exists()
...
Type checks and casts (polymorph
// Is Encounter.period.start a DateTime, and is it before today?
Encounter.period.start.is(DateTime)
  and Encounter.period.start.as(DateTime) < today()


String and set operations
// Bundle adheres to a specific profile
Bundle.meta.profile.contains('https://example.org/StructureDefinition/MyBundle')

// Code contained in a controlled set
Condition.code.coding.code in {'cvdd','mytp','mytd','mybd'}


Reference resolution (guarded)
// Resolve Composition.section entries and look for a specific code on the target
let refs := Composition.section.entry.reference
let targets := refs.resolve()
refs.exists() and targets.exists()
  and targets.code.coding.where(code = 'cvdd').exists()
```
---

## How DEMIS applies FHIRPath (preview of Parts II and III)

Right now we have implemented two core services that use FHIRPath to process notifications: Lifecycle-Validation-Service and
...
engine.setHostServices(new DemisEvaluationContext(bundle)); // custom resolve within bundle
