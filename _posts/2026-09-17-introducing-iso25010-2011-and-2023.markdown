...
Each requirement in ISO/IEC 25010 is represented by a dedicated skill. This skill acts as a bridge between the abstract quality model and the concrete project context. The AI can then apply the same assessment logic consistently across different projects, libraries, and components, which improves comparability and makes reuse much easier. 

### 3.2. An example skill for the requirement “Maintainability Reviewer Assess analyzability, modifiability, stability, testability, and reusability. Review architecture, code structure, tests, and technical debt. Identify changes that are hard or risky to make.” is shown below:
...
### 3.3. Score of a software system 
