...
**BUT!** If you only want to use ECIES with some defaults (like NIST curves) using Android, I can recommend to use [Tink](https://github.com/google/tink/blob/master/docs/JAVA-HOWTO.md#hybrid-encryption).
...
At that time it was clear, that the patient needed some form of autonomy over his or her own health information. So what does nearly everyone have nowadays? Yes, a smartphone!
...
Another point was to ensure that the payload of the communication between the client (E-Rezept App, PVS, AVS) and the actual destination on the server stays always encrypted, until it enters the TEE.
A normal HTTPS encryption would be terminated at the first endpoint, like the load balancer.
...
Have some ❤️, questions, feedback, or feel like 🤔 Reach out to me at [Twitter](https://twitter.com/t0bsdt) or [GitHub](https://github.com/tobias-schwerdtfeger).
